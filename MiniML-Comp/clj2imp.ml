...
let rec tr_expr (e: Clj.expression) (env: string STbl.t):
    Imp.sequence * Imp.expression * string list =
  let cpt = ref (-1) in
  let vars = ref [] in
  let new_var id =
    incr cpt;
  ...
  let rec translate (e: Clj.expression) (env: string STbl.t):
    ...
    | Clj.Match(e, cases) ->
      ...
      let rec compile_cases env = function
        | [] -> []
        | ((ctor, args), body)::rest ->
          let matched_id = struct_env ctor in
          let bind_vars =
            List.mapi (fun i x -> Imp.Set(x, Imp.array_get te1 (Imp.Int (i + 2)))) args
          in
          let is_body, te_body = translate body (List.fold_left (fun env x -> STbl.add x x env) env args) in
          [Imp.If(Imp.Binop(Eq, Imp.Var id_var, Imp.Int matched_id),
                 bind_vars @ is_body @ [Imp.Set(res_var, te_body)],
                 compile_cases env rest)]
      in
      is1 @ is2 @ compile_cases env cases, Imp.Var res_var
